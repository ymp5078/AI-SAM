    ...
    for batch in train_loader:
        ...
        if pbar is not None:
            ...
            if config['model']['name'] != 'sam':
                log_dict['attn_loss'] = model.loss_attn.item()
...
def main(config_, save_path, args):
    ...
    model = model.module
    if config['model']['name'] != 'sam':
        print('before:',model.pe_layer.positional_encoding_gaussian_matrix.mean(),model.prompt_encoder.not_a_point_embed.weight.mean())
    sam_checkpoint = torch.load(config['sam_checkpoint'])
    model.load_state_dict(sam_checkpoint, strict=False)
    if config['model']['name'] != 'sam':
        print('after:',model.pe_layer.positional_encoding_gaussian_matrix.mean(),model.prompt_encoder.not_a_point_embed.weight.mean())
